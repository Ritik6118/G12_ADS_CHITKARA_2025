...
public class Graph {
	...
	public void shortestPath_unweighted(int src,int des) {
//		Queue<Integer> q=new LinkedList<>();
		Queue<spPair> q=new LinkedList<>();
		HashSet<Integer> vis =new HashSet<>();
		
//		q.add(src);
		q.add(new spPair(src,""+src+" "));
		while(!q.isEmpty()) {
//			int n=q.poll();
			spPair p=q.poll();
			if(p.val==des) {
			...
			vis.add(p.val);
			for(int i:map.get(p.val).keySet()) {
				if(!vis.contains(i)) {
					q.add(new spPair(i,p.path+i+" "));
				}
			}
		}
	}
